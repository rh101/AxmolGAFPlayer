...
{
    _eventDispatcher->addCustomEventListener("ev_name", AX_CALLBACK_1(CustomEventTest::onCustomEvent, this));
}
...
void CustomEventTest::onCustomEvent(void* params)
{
    ...
    AXLOGD(
        "Catched event '{}' with parameter '{}'", 
        gafEvent->getParam(gaf::GAFTimelineAction::PI_EVENT_TYPE),
        gafEvent->getParam(gaf::GAFTimelineAction::PI_EVENT_DATA)
        );
}
